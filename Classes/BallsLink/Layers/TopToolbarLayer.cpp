...
bool TopToolbarLayer::init() {
  ...
  sprite->setPosition(service->getToolBarPosition(sprite));
...
void TopToolbarLayer::generateStar() {
  ...
  for(int i = 0; i < No_STAR; i++) {
    ...
    sprite->setAnchorPoint(Vec2(0, 1));
    auto star1Position = service->getStarPosition(getBg());
    if (i == 0) {
  ...
  starBatch->setPosition(service->getToolBarPosition(getBg()));
...
void TopToolbarLayer::generateAC() {
  ...
  for(int i = 0; i < No_HOLDER; i++) {
    ...
    sprite->setScale(service->getScale2());
    auto pos = service->getHolderPosition(getBg());
    ...
    auto ball = Ball::initBallWithoutAction();
...
void TopToolbarLayer::generateTimes() {
  auto service = Service::getInstance();
  auto pos = service->getToolBarPosition(getBg());
